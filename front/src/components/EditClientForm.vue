<template lang="html">
   ...
   <form @submit.prevent="validateBeforeSubmit" novalidate >
          ...
          <div :class="{'form-group': true, 'has-error': errors.has('adresseClient') }">
              <label>Adresse client (rue, ville, code postal)</label>
...
export default {
  ...
  methods: {
    ...
    formData: function (nomEntreprise, nomClient, prenomClient, adresseClient, telephoneClient, mailClient, secteurActivite) {
       this.$validator.validateAll().then((result) => {
        if (result) {
          ClientsService.editClient(this.$route.params.id, nomEntreprise, nomClient, prenomClient, adresseClient, telephoneClient, mailClient, secteurActivite)
          return;
        }
    });
    }
